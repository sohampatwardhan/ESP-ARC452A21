...
static const char s_control_page[] =
    ...
    "<div class=controls>"
    "<div class=controlRow id=tempField><div class=controlLabel>Temperature</div><div class=stepper><button onclick=tempStep(-1)>-</button><output id=temp>72 &deg;F</output><button onclick=tempStep(1)>+</button></div></div>"
    ...
    "function updateAge(){if(!updatedAt)return;let s=updatedAge+Math.floor((Date.now()-updatedAt)/1000),m=Math.floor(s/60),h=Math.floor(m/60);$('status').textContent='Last updated '+(s<60?'just now':m<60?m+' min ago':h+' hr ago')};"
    ...
    "$('mode').onclick=e=>{if(e.target.dataset.v){setSeg('mode',e.target.dataset.v);modeChanged();queue('mode '+e.target.dataset.v)}};"
    "$('fan').onclick=e=>{if(e.target.dataset.v){setSeg('fan',e.target.dataset.v);queue('fan '+e.target.dataset.v)}};"
    ...
    "function modeChanged(){$('tempField').classList.toggle('hidden',!tempModes[state.mode])};"
    "function tempBounds(){return unit=='c'?[16,32]:[60,90]};"
    "function tempText(v){return v+' '+deg+unit.toUpperCase()};"
    "function tempStep(d){let b=tempBounds(),v=Math.max(b[0],Math.min(b[1],(parseInt($('temp').textContent)||b[0])+d));$('temp').textContent=tempText(v);queue('temp '+v+' '+unit)};"
    ...
    "async function power(p){await cmd(p+(tempModes[state.mode]?' '+parseInt($('temp').textContent)+' '+unit:''))};"
    "async function refreshStatus(){let j=await fetch('/send?cmd=status').then(r=>r.json());show(j)};"
    "setInterval(updateAge,30000);setInterval(()=>refreshStatus().catch(()=>{}),15000);refreshStatus().then(()=>{ready=1}).catch(()=>{ready=1});"
    "</script></body></html>";
...
static const char s_settings_page[] =
    ...
    "<section><div class=cardHead><h2>Device</h2></div><div class=row><button class=secondary type=button onclick=rebootDevice()>Reboot</button></div><p class=inlineStatus id=deviceStatus></p></section>"
    ...
    "async function rebootDevice(){if(!confirm('Reboot device now?'))return;msg('deviceStatus',{ok:true,message:'Rebooting...'});msg('deviceStatus',await post('/command','reboot','text/plain'))}"
